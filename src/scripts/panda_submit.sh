...
# dirty hack for transfer of executable
echo ${bls_opt_the_command}  >> ${bls_opt_inputflstring}
...
if [[ ! -z "$bls_direct_arguments" ]]; then
   ...
   # for jsrun test
   bls_opt_the_command="$(basename ${bls_opt_the_command}) ${bls_direct_arguments}"
fi

# test whether it's a transfer job
if grep pegasus::transfer <<< ${bls_opt_the_command}; then
   bls_opt_the_command=$(tr ' ' "\n" <<< ${bls_opt_the_command} | sed -n '/pegasus-kickstart/,$p' | paste -s -d ' ' -)
fi

#input
if [[ ! -z "$bls_opt_stdin" && -f "$bls_opt_stdin" ]]; then
...
TMPJOBFILES=`mktemp /tmp/${jobID}_files.XXXXXXX`


echo "{\"in\" : {" > ${TMPJOBFILES}
#[[ -r $bls_opt_inputflstring ]] && cat $bls_opt_inputflstring | while read FILE; do echo -n \"$(base64 <<< $(basename ${FILE}))\" :  \"$(base64 < ${FILE})\", >> ${TMPJOBFILES} ; done 
[[ -r $bls_opt_inputflstring ]] && cat $bls_opt_inputflstring | while read FILE; do echo -n \"$(base64 <<< "$(basename ${FILE}).xz")\" :  \"$(xz -zc ${FILE} | base64 )\", >> ${TMPJOBFILES} ; done 

echo -e "},\n\"out\": { " >> ${TMPJOBFILES}
[[ -r $bls_opt_outputflstring ]] && cat $bls_opt_outputflstring | while read FILE; do echo -n \"$(base64 <<< $(basename ${FILE}))\" :  \"$(base64 < ${FILE})\", >> ${TMPJOBFILES} ; done 

echo -e "}\n,\"dir\": \"$bls_opt_workdir\"\n" >> ${TMPJOBFILES}
[[ -r "${bls_opt_stdin}" ]] && echo ,\"stdin\" : \"$(base64 ${bls_opt_stdin})\" >> ${TMPJOBFILES}
echo  "}" >> ${TMPJOBFILES}

sed -i 's/,}/}/' ${TMPJOBFILES}

curl -s -X POST -d @${TMPJOBFILES}  -H "Content-Type: application/json"  http://172.17.0.4:1234/joblogs/${jobID}/files 2>&1 >/dev/null
